...
void Server::Start(){
    ...
    m_clientConnection=std::make_unique<ClientConnection>(this);
    if(!m_clientConnection->initializeSocket(m_Port)){
        std::cerr<<"failed to initialzie"<<std::endl;
        return;
    }
    m_isRunnning=true;
    m_thread=std::thread(&Server::mainLoop, this);
}
...
void Server::mainLoop(){
    while(m_isRunning){
        std::string message;
        if(m_clientConnection->receiveMessage(message)){
            std::thread handler(&Server::handleMessage, this, message);
            handler.detach();
        }
    }
}
...
void Server::handleMessage(const std::string &message) {
    std::lock_guard<std::mutex> lock(m_mutex);  // Ensure thread-safe I2CManager access
